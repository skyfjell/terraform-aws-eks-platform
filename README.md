...
| <a name="input_config_karpenter"></a> [config\_karpenter](#input\_config\_karpenter) | Karpenter configuration. Karpenter does not handle cleaning itself up gracefully. We <br>    attempt to fix this by using a local-exec to wait, but this is not garunteed to work.<br><br>    Includes:<br>    - install: Will install the karpenter operator<br>    - enable\_provisioners: Will include the default helm release of the platform-system provisioner CR.<br>    - timeout: When tearing down the cluster, timeout (seconds) for waiting for the nodes to be torn down. | <pre>object({<br>    install             = optional(bool, true)<br>    enable_provisioners = optional(bool, true)<br>    timeout             = optional(number, 600)<br>  })</pre> | `{}` | no |
